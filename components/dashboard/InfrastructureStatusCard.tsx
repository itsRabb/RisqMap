...
const DetailPopup = ({ item, onClose }: { item: WaterLevelPost | PumpData, onClose: () => void }) => {
  ...
  if (isWaterLevel) {
    ...
    lastUpdated = getTimeAgo(new Date((item as PumpData).updated_at || Date.now()).toISOString(), lang); // Use updated_at for PumpData
  }
  ...
  const details = isWaterLevel
    ? [
      ...
      : [
      { label: t('infrastructure.location'), value: getPumpLocation(item as PumpData) || 'N/A', icon: <MapPin className="w-4 h-4 text-slate-400" /> },
      { label: t('infrastructure.pumpSystemStatus'), value: (item as PumpData).building_condition || 'Unknown', icon: <ShieldCheck className="w-4 h-4 text-slate-400" /> },
