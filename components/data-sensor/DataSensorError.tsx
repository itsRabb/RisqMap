...
export default function DataSensorError({ message }: { message: string }) {
    return (
        <div className="text-center bg-red-50 dark:bg-slate-800 p-8 rounded-xl border border-red-200 dark:border-red-500/20">
            ...
            <h3 className="text-xl font-semibold mb-2 text-slate-900 dark:text-white">Error Loading Data</h3>
            <p className="text-red-500 dark:text-red-400">Failed to load sensor data: {message}</p>
        </div>
    );
}
