...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = `cache:disaster_map:${request.url}`;
  ...
  try {
    ...
    // Apply rate limiting
    if (ip) {
      const { allowed, limit, remaining, reset } = await rateLimit(`disaster_map_rate_limit:${ip}`);
    ...
    const data = {
      message: "Data from Disaster Map API",
    ...
    if (ip) {
      const { limit, remaining, reset } = await rateLimit(`disaster_map_rate_limit:${ip}`);
